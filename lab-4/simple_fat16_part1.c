...
/**
 * @brief 将 buffer 中的数据写入到扇区号为 secnum 的扇区中
 *
 * @param fd      镜像文件指针
...
/**
 * @brief 从 fuse 中获取存储了文件系统元数据的 FAT16 指针
...
/**
 * @brief 从镜像文件中读取 BPB, 并计算所需的各类元数据
 *
 ...
 * then set the value of its FAT entry and the value of its first sector of
 *cluster.
 * ============================================================================
 **/

/**
 * @brief 读取指定簇号的簇的第一个扇区, 并给出相应的 FAT 表项和扇区号
 *
 * @param fat16_ins               文件系统元数据指针
 * @param ClusterN                簇号
 * @param FatClusEntryVal         输出参数, 对应簇的 FAT 表项
...
/**
 * @brief 从子目录 Dir 开始递归寻找 paths 对应的文件
 *
 * @param fat16_ins   文件系统元数据指针
                ...
                int curDepth, off_t *offset_dir)
{
    ...
    for (uint i = 1; Dir->DIR_Name[0] != 0x00; i++) {
        memcpy(Dir, &buffer[((i - 1) * BYTES_PER_DIR) % BYTES_PER_SECTOR],
               BYTES_PER_DIR);
                                     ...
                                     off_t offset, struct fuse_file_info *fi)
{
     ...
     *        读入内存, 再截取正确的部分填入 buffer. 我们不推荐那么做, 但不做
     *        强制要求, 如果你用这种方法实现了正确的功能, 同样能获得该部分全部分数.
     **/
...
/* (This is a new function) Add an entry according to specified sector and
 * offset
 * ==================================================================================
 * exp: sectorNum = 1000, offset = 1*BYTES_PER_DIR
 * The 1000th sector content before create: | Dir Entry(used)   |  Dir
 * Entry(free)  | ... | The 1000th sector content after create:  | Dir
 * Entry(used)   | Added Entry(used) | ... |
 * ==================================================================================
 */
/**
 * @brief 创建目录项
