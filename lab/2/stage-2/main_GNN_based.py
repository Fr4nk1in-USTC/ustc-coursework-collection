...
from parser.parser_GNN_based import parse_args
...
def train(args):
    ...
    for epoch in range(1, args.n_epoch + 1):
        ...
        for iter in range(1, n_cf_batch + 1):
            ...
            if np.isnan(cf_batch_loss.cpu().detach().numpy()):
                logging.info(
                    "ERROR (CF Training): Epoch {:04d} Iter {:04d} / {:04d} Loss is nan.".format(
    ...
    metrics_df.to_csv(
        args.save_dir + "/metrics{:d}.tsv".format(log_save_id), sep="\t", index=False
    )
