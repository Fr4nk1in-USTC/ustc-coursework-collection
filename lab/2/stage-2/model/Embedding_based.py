...
class Embedding_based(nn.Module):
    ...
    def calc_cf_loss(self, user_ids, item_pos_ids, item_neg_ids):
        ...
        item_pos_kg_embed = self.entity_embed(
            item_pos_ids
        )  # (cf_batch_size, embed_dim)
        item_neg_kg_embed = self.entity_embed(
            item_neg_ids
        )  # (cf_batch_size, embed_dim)

        # 8. 为 物品嵌入 注入 实体嵌入 的语义信息
        item_pos_cf_embed = inject(
            item_pos_embed, item_pos_kg_embed
        )  # (cf_batch_size, embed_dim)
        item_neg_cf_embed = inject(
            item_neg_embed, item_neg_kg_embed
        )  # (cf_batch_size, embed_dim)
        ...
        l2_loss = (
            ...
            + _L2_loss_mean(item_pos_cf_embed)
            + _L2_loss_mean(item_neg_cf_embed)
        )
    ...
    def calc_score(self, user_ids, item_ids):
        ...
        # 9. 为 物品嵌入 注入 实体嵌入 的语义信息
        item_cf_embed = inject(item_embed, item_kg_embed)  # (n_items, embed_dim)
        user_embed = torch.cat([user_embed, user_embed], dim=1)

        cf_score = torch.matmul(
            user_embed, item_cf_embed.transpose(0, 1)
        )  # (n_users, n_items)
