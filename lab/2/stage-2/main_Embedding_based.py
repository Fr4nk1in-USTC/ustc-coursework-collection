...
from parser.parser_Embedding_based import parse_args
from utils.log_helper import create_log_id, logging_config
from utils.metrics import calc_metrics_at_k
from utils.model_helper import early_stopping, load_model, save_model
...
def train(args):
    ...
    model = Embedding_based(
        args, data.n_users, data.n_items, data.n_entities, data.n_relations
    )
    ...
    for epoch in range(1, args.n_epoch + 1):
        ...
        for iter in range(1, n_batch + 1):
            ...
            if np.isnan(batch_loss.cpu().detach().numpy()):
                logging.info(
                    "ERROR: Epoch {:04d} Iter {:04d} / {:04d} Loss is nan.".format(
                        epoch, iter, n_batch
                    )
                )
                sys.exit()
        ...
        logging.info(
            "KG & CF Training: Epoch {:04d} Total Iter {:04d} | Total Time {:.1f}s | Iter Mean Loss {:.4f}".format(
