...
class PaillierPublicKey:
    ...
    def _convert_vector(plain_vector):
        if isinstance(plain_vector, list):
    ...
    *,
    ...
    for i in range(n_rows):
        encode_vector, vector_mapping = _target_encode(
            *raw_data[i], raw_pub_key=raw_pub_key, precision=precision
        )
...
def _target_encode(*vector, raw_pub_key, precision):
    ...
    for i, value in enumerate(vector):
        encode_number = phe.EncodedNumber.encode(
            raw_pub_key, value, precision=precision
        )
